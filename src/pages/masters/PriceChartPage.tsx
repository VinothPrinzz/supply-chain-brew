        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setCatIdx(Math.max(0, catIdx - 1))} disabled={catIdx === 0}>
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <span className="text-sm text-muted-foreground">{catIdx + 1} / {rateCategories.length}</span>
              <Button variant="outline" size="sm" onClick={() => setCatIdx(Math.min(rateCategories.length - 1, catIdx + 1))} disabled={catIdx === rateCategories.length - 1}>
        ...
        <CardContent>
          <table className="w-full text-sm">
            <thead><tr className="border-b">
              ...
              <th className="text-left py-2 px-3 font-medium text-muted-foreground">Pack</th>
              <th className="text-right py-2 px-3 font-medium text-muted-foreground">MRP</th>
              <th className="text-right py-2 px-3 font-medium text-muted-foreground">Rate ({cat})</th>
              <th className="text-right py-2 px-3 font-medium text-muted-foreground">GST%</th>
            </tr></thead>
            <tbody>
