...
import { routes, batches, indents, customers, products } from "@/data/mockData";
...
const PostIndentPage = () => {
  ...
  const pendingForRoute = indents.filter(
    (i) => i.routeId === selectedRoute && i.batchId === selectedBatch && i.status === "Pending"
  );
            ...
            {pendingForRoute.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead><tr className="border-b">
                  ...
                  <th className="text-left py-2 px-3 font-medium text-muted-foreground">Products</th>
                  <th className="text-right py-2 px-3 font-medium text-muted-foreground">Total Qty</th>
                </tr></thead>
                <tbody>
      ...
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { toast.success("Indents posted & route sheet generated (mock)"); setShowConfirm(false); }}>Confirm Post</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
